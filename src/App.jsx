...
// =========================================================
// 4. 拆分的獨立子元件 (LandingScreen, GameOverScreen 等)
// =========================================================

const LandingScreen = (props) => {
  const { setupMode, setSetupMode, setupPlayerCount, setSetupPlayerCount, setupTimeLimit, setSetupTimeLimit, setupAvatar, setSetupAvatar, setupName, setSetupName, localNames, setLocalNames, localPlayerTypes, setLocalPlayerTypes, localAvatars, setLocalAvatars, editingLocalPlayer, setEditingLocalPlayer, user, roomId, setRoomId, errorMsg, handleStartLocalGame, handleCreateRoom, handleJoinRoom } = props;
  ...
  const renderPlayerCountOptions = () => {
      ...
      for (let i = 0; i < opts.length; i++) {
          ...
          elements.push(
              <button key={num} onClick={() => setSetupPlayerCount(num)} className={`w-12 h-12 md:w-14 md:h-14 rounded-full text-xl md:text-2xl transition-all border-4 border-white ${bgClass}`}>
                  {num}
              </button>
          );
      }
  ...
  const renderTimeLimitOptions = () => {
      ...
      for (let i = 0; i < opts.length; i++) {
          ...
          elements.push(
              <button key={t.v} onClick={() => setSetupTimeLimit(t.v)} className={`px-4 py-2 md:px-5 md:py-3 rounded-[1.5rem] transition-all border-4 border-white text-sm md:text-base ${bgClass}`}>
                  {t.l}
              </button>
          );
      }
      ...
      for (let i = 0; i < setupPlayerCount; i++) {
          ...
          elements.push(
              <div key={i} className="flex flex-col items-center gap-1 md:gap-2">
                  <button onClick={() => setEditingLocalPlayer(i)} className={`w-10 h-10 md:w-12 md:h-12 rounded-full text-2xl md:text-3xl flex items-center justify-center bg-white transition-all border-4 ${bgClass}`}>
                      {localAvatars[i]}
                  </button>
      ...
      for (let j = 0; j < CHILD_AVATARS.length; j++) {
          ...
          elements.push(
              <button key={avatar} onClick={() => {
                  const newAvatars = [];
                  for(let k=0; k<localAvatars.length; k++) { newAvatars.push(localAvatars[k]); }
                  newAvatars[targetIdx] = avatar;
                  setLocalAvatars(newAvatars);
              }} className={`w-10 h-10 md:w-12 md:h-12 rounded-full text-2xl md:text-3xl flex items-center justify-center transition-all ${bgClass}`}>
                  {avatar}
              </button>
          );
      }
  ...
  const renderChildAvatarsJoin = () => {
      ...
      for (let j = 0; j < CHILD_AVATARS.length; j++) {
          ...
          elements.push(
              <button key={avatar} onClick={() => setSetupAvatar(avatar)} className={`w-12 h-12 md:w-16 md:h-16 rounded-full text-3xl md:text-4xl flex items-center justify-center bg-white transition-all ${bgClass}`}>
                  {avatar}
              </button>
          );
      }
                ...
                {setupMode === 'LOCAL' ? (
                  ...
                  <div className="w-full bg-sky-50 rounded-[2rem] p-5 md:p-6 border-4 border-white shadow-sm h-full flex flex-col justify-center">
                    ...
                    <div className="mb-4 w-full max-w-[200px] mx-auto">
                      <input type="text" value={setupName} onChange={(e) => {
                          const val = e.target.value;
                          setSetupName(val.substring(0, 6));
                      }} placeholder="輸入你的名字" className="w-full bg-white px-4 py-2.5 rounded-2xl text-center text-lg md:text-xl font-black border-4 border-sky-200 focus:border-amber-400 outline-none text-[#4a3424] shadow-inner transition-colors" />
                    </div>
            ...
            <div className="flex flex-col md:flex-row w-full gap-6">
              <div className="flex-1 flex flex-col justify-center gap-5">
                <div className="w-full">
                  ...
                  <input type="text" placeholder="A1B2C3" value={roomId} onChange={(e) => {
                      const val = e.target.value;
...
const TopBar = (props) => {
  const localTimeLeft = props.localTimeLeft;
...
const SquareDetailsModal = (props) => {
  const selectedSquareInfo = props.selectedSquareInfo;
  const setSelectedSquareInfo = props.setSelectedSquareInfo;
  const gameData = props.gameData;
  ...
  if (gameData.properties && gameData.properties[sq.id] !== undefined) {
      ...
      for(let i=0; i<gameData.players.length; i++){ if(gameData.players[i].id === oId){ owner = gameData.players[i]; break; } }
  }
...
const ActionModal = (props) => {
  const isTradeActive = props.isTradeActive;
  const tradeBuyer = props.tradeBuyer;
  const gameData = props.gameData;
  const myPlayer = props.myPlayer;
  const tradeBuyerMoney = props.tradeBuyerMoney;
  const handleRespondTrade = props.handleRespondTrade;
  const handleThrowBwaBwei = props.handleThrowBwaBwei;
  const handleFinishBwaBwei = props.handleFinishBwaBwei;
  const canBuy = props.canBuy;
  const handleBuyProperty = props.handleBuyProperty;
  const currentSquare = props.currentSquare;
  const handleEndTurn = props.handleEndTurn;
  const activePlayerIndex = props.activePlayerIndex;
  ...
  if (isTradeActive) {
      ...
      if (myPlayer && myPlayer.isAI) {
          ...
          let hasOwner = false;
          if (currentSquare && gameData.properties && gameData.properties[currentSquare.id] !== undefined) {
              hasOwner = true;
          }
          ...
          content = (
             <>
               ...
               <div className="flex flex-col gap-4 w-full mt-4 font-black">
                 {gameData.gameState === 'ACTION' && currentSquare && currentSquare.type === 'PROPERTY' && (!hasOwner) && (
...
const AssetManagerModal = (props) => {
  const showAssetManager = props.showAssetManager;
  const setShowAssetManager = props.setShowAssetManager;
  const myPlayer = props.myPlayer;
  const myProperties = props.myProperties;
  const sellProcess = props.sellProcess;
  const setSellProcess = props.setSellProcess;
  const handleMortgageTrust = props.handleMortgageTrust;
  const handleSellToBank = props.handleSellToBank;
  const initiatePlayerTrade = props.initiatePlayerTrade;
  const gameData = props.gameData;
  const isOfflineMode = props.isOfflineMode;
  const activePlayerIndex = props.activePlayerIndex;
...
const PlayerPiece = ({ p, pArrayIdx, activePlayersHere, gameData, activePlayerIndexInHere, isMyTurnOnThisCell, activePlayerIndex, myPlayer, inverseZoom, handleRollDice, setShowAssetManager, isTradeActive }) => {
  ...
  return (
      <div className={`absolute transition-all duration-500 ease-out pointer-events-auto flex flex-col items-center ${isActive ? 'z-50' : 'z-10'}`} style={{ transform: `translate(${tX}px, ${tY}px)` }}>
          ...
          <div onClick={(e) => { if (curPointer) { e.stopPropagation(); setShowAssetManager(true); } }} className={`w-20 h-20 bg-white rounded-full border-[6px] flex items-center justify-center text-[3rem] shadow-[0_8px_15px_rgba(0,0,0,0.1)] transition-all duration-500 ${bClasses} ${curClass}`}>
              {p.icon}
...
const BoardSquare = ({ sq, idx, gameData, activePlayerIndex, myPlayer, inverseZoom, handleRollDice, setShowAssetManager, setSelectedSquareInfo, isTradeActive, dragStatus }) => {
  const { row, col } = GRID_ORDER[idx];
  
  let ownerId = undefined;
  if (gameData.properties && gameData.properties[idx] !== undefined) {
      ownerId = gameData.properties[idx];
  }
  let owner = null;
  if (ownerId !== undefined && gameData.players) {
      for (let i = 0; i < gameData.players.length; i++) {
...
export default function App() {
  ...
  if (gameData.gameState === 'GAME_OVER') {
    return <GameOverScreen gameData={gameData} />;
  }
  ...
  const renderBoardSquares = function() {
      ...
      for (let idx = 0; idx < BOARD_SQUARES.length; idx++) {
          ...
          elements.push(
              <BoardSquare 
                 key={idx} sq={sq} idx={idx} gameData={gameData} activePlayerIndex={activePlayerIndex} myPlayer={myPlayer} 
                 inverseZoom={inverseZoom} handleRollDice={handleRollDice} setShowAssetManager={setShowAssetManager} 
                 setSelectedSquareInfo={setSelectedSquareInfo} isTradeActive={isTradeActive} dragStatus={dragStatus} 
              />
          );
      }
      return elements;
    ...
    <div className="h-screen w-screen bg-[#e0f2fe] overflow-hidden relative touch-none select-none font-black text-[#4a3424] flex flex-col">
      ...
      <div className="absolute right-4 bottom-8 md:right-6 md:bottom-10 flex flex-col items-end z-[150] pointer-events-auto">
        <div className={`flex flex-col items-end gap-3 transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-[500px] opacity-100 pb-3 pointer-events-auto' : 'max-h-0 opacity-0 pointer-events-none'}`}>
          <button onClick={function(){ setZoom(function(z){ return Math.min(z + 0.1, 1.5); }); setIsMenuOpen(false); }} className="h-14 px-5 bg-white/95 backdrop-blur-md rounded-full border-4 border-sky-100 flex items-center gap-3 font-black text-sky-600 shadow-lg active:scale-95 transition-all">放大畫面 <ZoomIn size={24} /></button>
          <button onClick={function(){ focusOnCurrentPlayer(); setIsMenuOpen(false); }} className="h-14 px-5 bg-white/95 backdrop-blur-md rounded-full border-4 border-sky-100 flex items-center gap-3 font-black text-sky-600 shadow-lg active:scale-95 transition-all">找回角色 <Target size={24} /></button>
          <button onClick={function(){ setZoom(function(z){ return Math.max(z - 0.1, 0.4); }); setIsMenuOpen(false); }} className="h-14 px-5 bg-white/95 backdrop-blur-md rounded-full border-4 border-sky-100 flex items-center gap-3 font-black text-sky-600 shadow-lg active:scale-95 transition-all">縮小畫面 <ZoomOut size={24} /></button>
          <button onClick={function(){ setIsFullMapMode(!isFullMapMode); setIsMenuOpen(false); }} className={`h-14 px-5 backdrop-blur-md rounded-full border-4 flex items-center gap-3 font-black shadow-lg active:scale-95 transition-all ${isFullMapMode ? 'bg-sky-400 text-white border-white' : 'bg-white/95 text-sky-600 border-sky-100'}`}>{isFullMapMode ? '關閉全覽' : '全覽地圖'}<Menu size={24} /></button>
          <button onClick={function(){ setIsMuted(!isMuted); }} className="h-14 px-5 bg-white/95 backdrop-blur-md rounded-full border-4 border-amber-100 flex items-center gap-3 font-black text-amber-500 shadow-lg active:scale-95 transition-all">{isMuted ? '開啟音效' : '關閉音效'} {isMuted ? <VolumeX size={24} /> : <Volume2 size={24} />}</button>
          <button onClick={function(){ setShowExitConfirm(true); setIsMenuOpen(false); }} className="h-14 px-5 bg-white/95 backdrop-blur-md rounded-full border-4 border-rose-100 flex items-center gap-3 font-black text-rose-500 shadow-lg active:scale-95 transition-all">離開遊戲 <LogOut size={24} /></button>
        </div>
        <button onClick={function(){ setIsMenuOpen(!isMenuOpen); }} className={`w-16 h-16 md:w-20 md:h-20 rounded-full shadow-2xl flex items-center justify-center border-[5px] transition-all duration-300 transform ${isMenuOpen ? 'bg-sky-400 border-white rotate-90 scale-95 text-white' : 'bg-white/90 backdrop-blur-md border-sky-100 text-sky-500 hover:scale-105 active:scale-95'}`}>
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-sky-900/40 backdrop-blur-sm pointer-events-auto">
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 max-w-sm w-full mx-4 animate-in zoom-in-95 spin-in-1 border-[8px] border-rose-100">
            ...
            <div className="flex gap-4 w-full mt-4">
              <button onClick={function(){ setShowExitConfirm(false); }} className="flex-1 py-4 bg-slate-100 hover:bg-slate-200 text-slate-500 text-xl rounded-[2rem] transition-colors border-4 border-white shadow-sm">按錯了啦</button>
              <button onClick={function(){ window.location.reload(); }} className="flex-1 py-4 bg-rose-400 hover:bg-rose-300 text-white text-xl rounded-[2rem] shadow-[0_5px_0_0_#e11d48] active:translate-y-[5px] active:shadow-none transition-all border-4 border-white">確定離開</button>
            </div>
          </div>
        </div>
      )}
      ...
      {renderDiceRollingAnimation()}
      {renderBwaBweiRollingAnimation()}
