...
// =========================================================
// 1. 全域常數與純函數 (安全放置於頂部，杜絕 ReferenceError)
// =========================================================
...
export default function App() {
  ...
  const [localAvatars, setLocalAvatars] = useState([...CHILD_AVATARS].slice(0, 6));
  ...
  const myProperties = Object.keys(propertiesObj).filter(key => propertiesObj[key] === activePlayerIndex).map(Number);
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const onStart = (e) => {
      dragStatus.current.isDragging = true; dragStatus.current.moved = false; 
      dragStatus.current.initX = e.clientX; dragStatus.current.initY = e.clientY;
      dragStatus.current.startX = e.clientX - manualOffset.x; dragStatus.current.startY = e.clientY - manualOffset.y;
    };
    const onMove = (e) => {
      if (!dragStatus.current.isDragging) return;
      if (Math.abs(e.clientX - dragStatus.current.initX) > 5 || Math.abs(e.clientY - dragStatus.current.initY) > 5) dragStatus.current.moved = true;
      setManualOffset({ x: e.clientX - dragStatus.current.startX, y: e.clientY - dragStatus.current.startY });
    };
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (firebaseConfig.apiKey && (firebaseConfig.apiKey.includes("請貼上") || firebaseConfig.apiKey.includes("填入"))) setErrorMsg("請確認 Firebase API_KEY！(單機模式可忽略)");
        else setErrorMsg("網路連線失敗，請檢查金鑰。");
      }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    return onSnapshot(roomRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data(); setGameData(data);
  ...
  const handleStartLocalGame = async () => {
    ...
    const initialPlayers = Array.from({ length: setupPlayerCount }).map((_, i) => ({
      id: i, name: (localNames[i] || '').trim() || `玩家 ${i + 1}`, icon: localAvatars[i], color: ['bg-sky-300', 'bg-rose-300', 'bg-emerald-300', 'bg-purple-300', 'bg-orange-300', 'bg-pink-300'][i % 6],
      pos: 0, money: BASE_MONEY, trust: BASE_TRUST, inJail: false, jailRoundsLeft: 0, isBankrupt: false, uid: `local_player_${i}`, isAI: localPlayerTypes[i] === 'AI'
    }));
  ...
  const handleCreateRoom = async () => {
    ...
    const initialPlayers = Array.from({ length: setupPlayerCount }).map((_, i) => ({
      id: i, name: i === 0 ? ((setupName || '').trim() || '房主') : `玩家 ${i + 1}`, icon: i === 0 ? setupAvatar : '⏳', color: ['bg-sky-300', 'bg-rose-300', 'bg-emerald-300', 'bg-purple-300', 'bg-orange-300', 'bg-pink-300'][i % 6],
  ...
  const handleLandOnSquare = async () => {
    ...
    if (bkResult.changed && bkResult.newPlayers[activePlayerIndex].isBankrupt) { playSound('bad', isMuted); msg += `\n\n🚨 哎呀！資金或信用歸零，你出局了！`; nextState = 'END_TURN'; }
