...
const GRID_ORDER = Array.from({ length: 40 }).map((_, i) => {
...
const getFirebaseConfig = () => {
  ...
  } catch (e) { console.warn("使用預設金鑰"); }
  return { apiKey: "AIzaSyBNN-5xswc1tq_Y5ymWMVGFldZRfpvsVZM", authDomain: "da-xin-wong.firebaseapp.com", projectId: "da-xin-wong", storageBucket: "da-xin-wong.appspot.com", messagingSenderId: "72871979370", appId: "1:72871979370:web:97caab1074d5f1e8f9dd13" };
};
...
const LandingScreen = ({ setupMode, setSetupMode, setupPlayerCount, setSetupPlayerCount, setupTimeLimit, setSetupTimeLimit, setupAvatar, setSetupAvatar, setupName, setSetupName, localNames, setLocalNames, localPlayerTypes, setLocalPlayerTypes, localAvatars, setLocalAvatars, editingLocalPlayer, setEditingLocalPlayer, user, roomId, setRoomId, errorMsg, handleStartLocalGame, handleCreateRoom, handleJoinRoom }) => (
  <div className="min-h-screen w-screen bg-[#e0f2fe] flex flex-col items-center justify-center p-4 md:p-6 text-[#4a3424] overflow-x-hidden absolute inset-0 font-black">
    ...
    <h1 className="text-5xl md:text-[4.5rem] font-black mb-4 md:mb-6 text-sky-500 tracking-widest drop-shadow-[0_6px_0_rgba(2,132,199,0.2)] text-center leading-tight">大信翁<span className="block text-xl md:text-2xl text-rose-400 mt-1 tracking-normal">Candy Bubble Edition 🍬</span></h1>
    {errorMsg && <div className="mb-4 bg-rose-100 text-rose-700 p-3 rounded-2xl border-4 border-rose-300 shadow-sm">{String(errorMsg)}</div>}
            ...
            <div className="flex-1 flex flex-col justify-center gap-4">
              <div className="w-full">
                ...
                <div className="flex flex-wrap justify-center gap-2 md:gap-3">
                  {[2, 3, 4, 5, 6].map(num => <button key={num} onClick={() => setSetupPlayerCount(num)} className={`w-12 h-12 md:w-14 md:h-14 rounded-full text-xl md:text-2xl transition-all border-4 border-white ${setupPlayerCount === num ? 'bg-amber-400 text-amber-900 scale-110 shadow-[0_4px_0_0_#d97706]' : 'bg-sky-100 text-sky-600 hover:bg-sky-200 shadow-sm'}`}>{num}</button>)}
                </div>
              </div>
              ...
              <div className="w-full">
                ...
                <div className="flex flex-wrap justify-center gap-2 md:gap-3">
                  {[{ l: '5 分', v: 300 }, { l: '10 分', v: 600 }, { l: '20 分', v: 1200 }, { l: '不限時', v: -1 }].map(t => <button key={t.v} onClick={() => setSetupTimeLimit(t.v)} className={`px-4 py-2 md:px-5 md:py-3 rounded-[1.5rem] transition-all border-4 border-white text-sm md:text-base ${setupTimeLimit === t.v ? 'bg-pink-400 text-pink-900 shadow-[0_4px_0_0_#db2777]' : 'bg-sky-100 text-sky-600 hover:bg-sky-200 shadow-sm'}`}>{t.l}</button>)}
                </div>
              </div>
            </div>
              ...
              {setupMode === 'LOCAL' ? (
                <div className="w-full bg-sky-50 rounded-[2rem] p-4 md:p-5 border-4 border-white shadow-sm h-full flex flex-col">
                  ...
                  <div className="flex flex-wrap justify-center gap-2 max-h-24 md:max-h-32 overflow-y-auto p-2 bg-white rounded-[1.5rem] border-2 border-sky-100 custom-scrollbar mt-auto">
                    {CHILD_AVATARS.map(avatar => {
                      ...
                      return <button key={avatar} onClick={() => { const nA = [...localAvatars]; nA[tIdx] = avatar; setLocalAvatars(nA); }} className={`w-10 h-10 md:w-12 md:h-12 rounded-full text-2xl md:text-3xl flex items-center justify-center transition-all ${localAvatars[tIdx] === avatar ? 'bg-amber-100 border-4 border-amber-400 scale-110' : 'bg-slate-50 border-2 border-transparent hover:bg-sky-100'}`}>{avatar}</button>;
                    })}
                  </div>
                </div>
              ) : (
                <div className="w-full bg-sky-50 rounded-[2rem] p-5 md:p-6 border-4 border-white shadow-sm h-full flex flex-col justify-center">
                  ...
                  <div className="flex flex-wrap justify-center gap-2 md:gap-3 max-h-36 md:max-h-48 overflow-y-auto p-2 custom-scrollbar">
                    {CHILD_AVATARS.map(avatar => <button key={avatar} onClick={() => setSetupAvatar(avatar)} className={`w-12 h-12 md:w-16 md:h-16 rounded-full text-3xl md:text-4xl flex items-center justify-center bg-white transition-all ${setupAvatar === avatar ? 'border-4 border-amber-400 scale-110 shadow-md' : 'border-2 border-sky-100 hover:bg-sky-100'}`}>{avatar}</button>)}
                  </div>
                </div>
              )}
            </div>
...
const SquareDetailsModal = ({ selectedSquareInfo, setSelectedSquareInfo, gameData }) => {
        ...
        {sq.type === 'PROPERTY' && (
          <div className="flex flex-col gap-3 w-full">
            <div className="flex justify-between items-center bg-sky-50 p-4 rounded-[1.5rem] border-4 border-white shadow-sm">
              <span className="text-sky-800 text-lg">購買需要</span><span className="font-black text-sky-600 text-2xl">${sq.price}</span>
            </div>
            <div className="flex justify-between items-center bg-rose-50 p-4 rounded-[1.5rem] border-4 border-white shadow-sm">
              <span className="text-rose-800 text-lg">過路費</span><span className="font-black text-rose-500 text-2xl">${rentPrice}</span>
            </div>
            <div className="flex justify-between items-center bg-amber-50 p-4 rounded-[1.5rem] border-4 border-white shadow-sm">
              <span className="text-amber-800 text-lg">信用門檻</span><span className="font-black text-amber-500 text-2xl flex items-center gap-1"><Star size={24} fill="currentColor" /> {sq.reqTrust}</span>
            </div>
            <div className="mt-4 p-5 rounded-[2rem] border-[4px] border-dashed border-slate-200 text-center bg-slate-50 relative overflow-hidden">
              ...
              {owner ? (
                <div className="font-black text-3xl flex items-center justify-center gap-3"><span className="text-5xl drop-shadow-md">{owner.icon}</span> <span className="text-emerald-600">{owner.name}</span></div>
...
const AssetManagerModal = ({ showAssetManager, setShowAssetManager, myPlayer, myProperties, sellProcess, setSellProcess, handleMortgageTrust, handleSellToBank, initiatePlayerTrade, gameData, isOfflineMode, activePlayerIndex }) => {
...
const ActionModal = ({ isTradeActive, tradeBuyer, gameData, myPlayer, tradeBuyerMoney, handleRespondTrade, handleThrowBwaBwei, handleFinishBwaBwei, canBuy, handleBuyProperty, currentSquare, handleEndTurn, activePlayerIndex }) => {
  ...
  if (isTradeActive) {
      ...
      if (myPlayer?.isAI) {
          ...
          const hasOwner = gameData.properties?.[currentSquare?.id] !== undefined;
          content = (
             <>
...
const PlayerPiece = ({ p, pArrayIdx, activePlayersHere, gameData, activePlayerIndexInHere, isMyTurnOnThisCell, activePlayerIndex, myPlayer, inverseZoom, handleRollDice, setShowAssetManager, isTradeActive }) => {
  const isActive = gameData.currentPlayerIdx === p.id;
  let tX = 0; let tY = 0;
  if (!isActive) {
      let myInactiveIdx = pArrayIdx;
      if (activePlayerIndexInHere !== -1 && pArrayIdx > activePlayerIndexInHere) myInactiveIdx = pArrayIdx - 1;
              ...
              <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 z-[200]">
                <div className="flex flex-col items-center gap-3 animate-in slide-in-from-bottom-4 duration-300" style={{ transform: `scale(${inverseZoom})`, transformOrigin: 'bottom center' }}>
                  {myPlayer?.isAI ? (
                      ...
                      <button onClick={handleRollDice} className="whitespace-nowrap px-8 py-4 bg-sky-400 hover:bg-sky-300 text-white rounded-[2rem] font-black text-3xl shadow-[0_8px_0_0_#0284c7,0_10px_20px_rgba(0,0,0,0.15)] active:shadow-none active:translate-y-[8px] active:border-b-0 transition-all flex items-center gap-3 border-[4px] border-white animate-bounce"><Dice5 size={32} strokeWidth={3} /> 擲骰子</button>
                  )}
                </div>
              </div>
          )}
...
const MapSquareNode = ({ sq, idx, gameData, activePlayerIndex, myPlayer, inverseZoom, handleRollDice, setShowAssetManager, setSelectedSquareInfo, isTradeActive, dragStatus }) => {
      ...
      <div style={{ display: 'contents' }}>
          <div onClick={() => { if (!dragStatus.current.moved) setSelectedSquareInfo(idx); }} className={`rounded-[1.5rem] relative flex flex-col overflow-hidden shadow-sm z-10 border-4 border-b-[8px] cursor-pointer hover:scale-[1.03] transition-transform pointer-events-auto ${borderClass}`} style={{ gridRow: row, gridColumn: col }}>
              ...
              <div className={contentClass}>
                  ...
                  {sq.reqTrust > 0 && <div className="mt-1 bg-amber-50 text-amber-600 text-xs font-black px-2 py-0.5 rounded-full border-2 border-amber-300 flex items-center justify-center gap-1 shadow-sm"><Star size={14} fill="currentColor" /> {sq.reqTrust} 點</div>}
              </div>
          </div>
          ...
          <div className={`flex items-center justify-center relative ${activePlayerIndexInHere !== -1 ? 'z-[100]' : 'z-20 pointer-events-none'}`} style={{ gridRow: row, gridColumn: col }}>
              {activePlayersHere.map((p, pArrayIdx) => (
                  <PlayerPiece key={p.id} p={p} pArrayIdx={pArrayIdx} activePlayersHere={activePlayersHere} gameData={gameData} activePlayerIndexInHere={activePlayerIndexInHere} isMyTurnOnThisCell={isMyTurnOnThisCell} activePlayerIndex={activePlayerIndex} myPlayer={myPlayer} inverseZoom={inverseZoom} handleRollDice={handleRollDice} setShowAssetManager={setShowAssetManager} isTradeActive={isTradeActive} />
              ))}
          </div>
      </div>
  );
...
export default function App() {
  ...
  const [localAvatars, setLocalAvatars] = useState([...CHILD_AVATARS]);
  ...
  const myPlayer = gameData.players.find(p => p.id === activePlayerIndex) || null;
  const currentSquare = myPlayer?.pos !== undefined ? BOARD_SQUARES[myPlayer.pos] : null;
  
  const myMoney = myPlayer?.money ? Number(myPlayer.money) : 0;
  const myTrust = myPlayer?.trust ? Number(myPlayer.trust) : 0;
  const reqMoney = currentSquare?.price ? Number(currentSquare.price) : 0;
  const reqTrust = currentSquare?.reqTrust ? Number(currentSquare.reqTrust) : 0;
  
  const wRatio = viewportSize.w / MAP_SIZE;
  const hRatio = viewportSize.h / MAP_SIZE;
  const displayZoom = isFullMapMode ? (Math.min(wRatio, hRatio) * 0.9) : zoom;
  const inverseZoom = 1 / displayZoom; 
  
  const canBuy = Boolean(currentSquare && myMoney >= reqMoney && myTrust >= reqTrust);
  const myProperties = Object.keys(gameData.properties || {}).filter(k => gameData.properties[k] === activePlayerIndex).map(Number);
  const safeDice = displayDice || [1, 1];

  const pendingBuyerId = gameData.pendingTrade?.buyerIdx ?? -1;
  const isTradeActive = Boolean(gameData.pendingTrade && (isOfflineMode || pendingBuyerId === activePlayerIndex));
  const tradeBuyer = gameData.players.find(p => p.id === pendingBuyerId) || null;
  const tradeBuyerMoney = tradeBuyer?.money ? Number(tradeBuyer.money) : 0;
  ...
  useEffect(() => {
    ...
    bgmRef.current.loop = true; bgmRef.current.volume = 0.1; 
  ...
  useEffect(() => {
    ...
    else if (bgmStarted) { const p = bgmRef.current.play(); if (p !== undefined) p.catch(() => {}); }
  }, [isMuted, bgmStarted]);

  useEffect(() => {
    const startAudio = () => setBgmStarted(true);
  ...
  const focusOnCurrentPlayer = useCallback(() => {
    ...
    const currP = gameData.players.find(p => p.id === gameData.currentPlayerIdx);
    if (!currP) return;
    const { row, col } = GRID_ORDER[currP.pos];
    const CELL_SIZE = MAP_SIZE / 11;
    setCameraOffset({ x: viewportSize.w / 2 - ((col - 1) * CELL_SIZE + CELL_SIZE / 2) * displayZoom, y: viewportSize.h * 0.65 - ((row - 1) * CELL_SIZE + CELL_SIZE / 2) * displayZoom });
    setManualOffset({ x: 0, y: 0 }); 
  }, [gameData.players, gameData.currentPlayerIdx, viewportSize, displayZoom]);

  useEffect(() => {
    if (appPhase !== 'GAME' || isFullMapMode) {
      if (isFullMapMode) { setCameraOffset({ x: viewportSize.w / 2 - (MAP_SIZE / 2) * displayZoom, y: viewportSize.h / 2 - (MAP_SIZE / 2) * displayZoom }); setManualOffset({ x: 0, y: 0 }); }
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const onStart = (e) => { dragStatus.current.isDragging = true; dragStatus.current.moved = false; dragStatus.current.initX = e.clientX; dragStatus.current.initY = e.clientY; dragStatus.current.startX = e.clientX - manualOffset.x; dragStatus.current.startY = e.clientY - manualOffset.y; };
    const onMove = (e) => { if (!dragStatus.current.isDragging) return; if (Math.abs(e.clientX - dragStatus.current.initX) > 5 || Math.abs(e.clientY - dragStatus.current.initY) > 5) dragStatus.current.moved = true; setManualOffset({ x: e.clientX - dragStatus.current.startX, y: e.clientY - dragStatus.current.startY }); };
    const onEnd = () => { dragStatus.current.isDragging = false; };
    el.addEventListener('pointerdown', onStart); window.addEventListener('pointermove', onMove); window.addEventListener('pointerup', onEnd);
    return () => { el.removeEventListener('pointerdown', onStart); window.removeEventListener('pointermove', onMove); window.removeEventListener('pointerup', onEnd); };
  }, [manualOffset]);
  ...
  const handleStartLocalGame = async () => {
    playSound('win', isMuted); setIsOfflineMode(true);
    const initialPlayers = Array.from({ length: setupPlayerCount }).map((_, i) => ({
      id: i, name: (localNames[i] || '').trim() || `玩家 ${i + 1}`, icon: localAvatars[i], 
      color: ['bg-sky-300', 'bg-rose-300', 'bg-emerald-300', 'bg-purple-300', 'bg-orange-300', 'bg-pink-300'][i % 6],
      pos: 0, money: BASE_MONEY, trust: BASE_TRUST, inJail: false, jailRoundsLeft: 0, isBankrupt: false,
      uid: `local_player_${i}`, isAI: localPlayerTypes[i] === 'AI'
    }));
    setGameData({ players: initialPlayers, currentPlayerIdx: 0, gameState: 'IDLE', roomId: 'LOCAL', timeLeft: setupTimeLimit, properties: {}, actionMessage: '', remainingSteps: 0, diceVals: [1, 1], bwaBweiResults: [], pendingTrade: null });
    setRoomId('單機同樂'); setAppPhase('GAME'); setLocalTimeLeft(setupTimeLimit);
  };

  const handleCreateRoom = async () => {
    if (!user) return;
    playSound('win', isMuted); setIsOfflineMode(false);
    const id = Math.random().toString(36).substring(2, 8).toUpperCase();
    const initialPlayers = Array.from({ length: setupPlayerCount }).map((_, i) => ({
      id: i, name: i === 0 ? ((setupName || '').trim() || '房主') : `玩家 ${i + 1}`, icon: i === 0 ? setupAvatar : '⏳', 
      color: ['bg-sky-300', 'bg-rose-300', 'bg-emerald-300', 'bg-purple-300', 'bg-orange-300', 'bg-pink-300'][i % 6],
      pos: 0, money: BASE_MONEY, trust: BASE_TRUST, inJail: false, jailRoundsLeft: 0, isBankrupt: false, uid: i === 0 ? user.uid : null 
    }));
    try {
      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', id), { players: initialPlayers, currentPlayerIdx: 0, gameState: 'IDLE', roomId: id, timeLeft: setupTimeLimit, properties: {}, actionMessage: '', remainingSteps: 0, diceVals: [1, 1], bwaBweiResults: [], pendingTrade: null });
      setRoomId(id); setIsHost(true); setMyPlayerIndex(0); setAppPhase('GAME'); setLocalTimeLeft(setupTimeLimit);
    } catch (e) { setErrorMsg("建立失敗，請確認設定。"); }
  };
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      if (existingSlot !== -1) { setMyPlayerIndex(existingSlot); setAppPhase('GAME'); if (data.timeLeft !== -1) setLocalTimeLeft(data.timeLeft); return; }
      ...
      setMyPlayerIndex(emptySlot); setAppPhase('GAME'); if (data.timeLeft !== -1) setLocalTimeLeft(data.timeLeft);
  ...
  useEffect(() => {
    if (gameData.gameState === 'ROLLING') {
      const interval = setInterval(() => { setDisplayDice([Math.floor(Math.random() * 6) + 1, Math.floor(Math.random() * 6) + 1]); }, 100);
      return () => clearInterval(interval);
    } else { setDisplayDice(gameData.diceVals || [1, 1]); }
    ...
    const moveTimer = setTimeout(async () => {
      try {
        ...
        if (gameData.remainingSteps > 0) {
          ...
          let newPos = (player.pos + 1) % 40; let newMoney = player.money; let msg = gameData.actionMessage || '';
          ...
          let nextState = 'ACTION'; let msg = gameData.actionMessage || '';
          const nextPlayers = [...gameData.players];
          const currPlayerClone = { ...nextPlayers[activePlayerIndex] };

          if (sq.type === 'START') { playSound('coin', isMuted); msg += `經過起點領零用錢囉！✨`; nextState = 'END_TURN'; }
          else if (sq.type === 'TAX') { playSound('bad', isMuted); currPlayerClone.money -= sq.amount; msg += `💸 繳納 ${sq.name} $${sq.amount}！`; nextState = 'END_TURN'; }
          else if (sq.type === 'CHANCE_GOOD' || sq.type === 'CHANCE_BAD') {
            const cardPool = sq.type === 'CHANCE_GOOD' ? GOOD_CARDS : BAD_CARDS;
            const card = cardPool[Math.floor(Math.random() * cardPool.length)];
            msg += `【 ${card.desc} 】\n\n`;
            if (card.goToJail) { playSound('bad', isMuted); currPlayerClone.pos = 10; currPlayerClone.inJail = true; currPlayerClone.jailRoundsLeft = -1; msg += `好好的懺悔反省 🙏\n請誠心擲杯問神明。`; nextState = 'JAIL_BWA_BWEI'; }
            else { if (card.effectM > 0 || card.effectT > 0) playSound('win', isMuted); else playSound('bad', isMuted); currPlayerClone.money += card.effectM; currPlayerClone.trust += card.effectT; msg += `資金 ${card.effectM > 0 ? '+'+card.effectM : card.effectM}\n信用 ${card.effectT > 0 ? '+'+card.effectT : card.effectT}`; }
            if (!card.goToJail) nextState = 'END_TURN';
          }
          else if (sq.type === 'GO_TO_JAIL' || sq.id === 30 || sq.id === 10) { playSound('bad', isMuted); currPlayerClone.pos = 10; currPlayerClone.inJail = true; currPlayerClone.jailRoundsLeft = -1; msg += `好好的懺悔反省 🙏\n請誠心擲杯問神明。`; nextState = 'JAIL_BWA_BWEI'; }
          else if (sq.type === 'PROPERTY') {
            const ownerId = gameData.properties?.[sq.id];
            if (ownerId !== undefined && ownerId !== activePlayerIndex) {
              const owner = nextPlayers[ownerId];
              if (!owner.inJail && !owner.isBankrupt) { playSound('bad', isMuted); const rent = Math.floor(sq.price * 0.4); currPlayerClone.money -= rent; nextPlayers[ownerId] = { ...nextPlayers[ownerId], money: nextPlayers[ownerId].money + rent }; msg += `踩到 ${owner.name} 的地盤，\n付過路費 $${rent} 給他吧！`; }
              else { playSound('win', isMuted); msg += `幸運！ ${owner.name} ${owner.inJail ? '正在反省' : '已出局'}，免付過路費！ 🎉`; }
              nextState = 'END_TURN';
            } else if (ownerId === undefined) { playSound('click', isMuted); msg += `來到空地：${sq.name} 🏕️`; }
            else { playSound('click', isMuted); msg += `來到自己的 ${sq.name}，\n巡視一下產業！ 😎`; nextState = 'END_TURN'; }
          }
          else if (sq.type === 'FREE_PARKING') { playSound('click', isMuted); msg += `平靜的一回合，\n培養品德心性的好地方 🍵`; nextState = 'END_TURN'; }
          else { playSound('click', isMuted); msg += `在 ${sq.name} 休息一天 💤`; nextState = 'END_TURN'; }

          nextPlayers[activePlayerIndex] = currPlayerClone;
          let hasBankruptcies = false;
          const checkedPlayers = nextPlayers.map(p => {
             if (!p.isBankrupt && (p.money < 0 || p.trust <= 0)) { hasBankruptcies = true; return { ...p, isBankrupt: true }; }
             return p;
          });

          if (hasBankruptcies && checkedPlayers[activePlayerIndex].isBankrupt) { playSound('bad', isMuted); msg += `\n\n🚨 哎呀！資金或信用歸零，你出局了！`; nextState = 'END_TURN'; }

          let nextProps = { ...gameData.properties };
          if (hasBankruptcies) {
              const bankruptIds = checkedPlayers.filter(p => p.isBankrupt).map(p => p.id);
              Object.keys(nextProps).forEach(k => { if(bankruptIds.includes(nextProps[k])) delete nextProps[k]; });
          }
          await syncGameData({ players: checkedPlayers, properties: nextProps, gameState: nextState, actionMessage: msg, bwaBweiResults: [] });
        }
      } catch (e) {}
    }, 350);
  ...
  useEffect(() => {
    ...
    if (gameData.gameState === 'BWA_BWEI_ROLLING' && gameData.currentPlayerIdx === activePlayerIndex) {
      const timer = setTimeout(async () => {
        ...
        const res = rand < 0.5 ? 'HOLY' : (rand < 0.75 ? 'LAUGH' : 'YIN');
  ...
  const handleSellToBank = useCallback(async (sqId, price) => {
    ...
    const nextProps = { ...gameData.properties }; delete nextProps[sqId];
  ...
  const handleRespondTrade = useCallback(async (accept) => {
    ...
    if (accept) {
        playSound('coin', isMuted);
        const nextPlayers = [...gameData.players];
        nextPlayers[trade.sellerIdx] = { ...nextPlayers[trade.sellerIdx], money: nextPlayers[trade.sellerIdx].money + trade.price };
        nextPlayers[trade.buyerIdx] = { ...nextPlayers[trade.buyerIdx], money: nextPlayers[trade.buyerIdx].money - trade.price };
  ...
  const handleEndTurn = async () => {
    ...
    let attempts = 0;
    do { nextIdx = (nextIdx + 1) % nextPlayers.length; attempts++; } while (nextPlayers[nextIdx].isBankrupt && attempts < 10);

    const nextPlayerClone = { ...nextPlayers[nextIdx] };
    let nextState = 'IDLE'; let msg = '';
    ...
    if (nextPlayerClone.inJail && nextPlayerClone.jailRoundsLeft > 0) {
        ...
        if (nextPlayerClone.jailRoundsLeft === 0) {
            nextPlayerClone.money -= 500; nextPlayerClone.inJail = false; msg = `🌟 ${nextPlayerClone.name} 反省期滿，扣除罰金 $500。\n離開反省泡泡，可正常行動囉！`;
        } else {
            nextState = 'END_TURN'; msg = `🔒 ${nextPlayerClone.name} 仍在反省泡泡中...\n(還要等 ${nextPlayerClone.jailRoundsLeft} 輪喔)`;
        }
    }
    nextPlayers[nextIdx] = nextPlayerClone;

    let hasBankruptcies = false;
    const bkPlayers = nextPlayers.map(p => {
       if (!p.isBankrupt && (p.money < 0 || p.trust <= 0)) { hasBankruptcies = true; return { ...p, isBankrupt: true }; }
       return p;
    });

    if (bkPlayers[nextIdx].isBankrupt && nextPlayerClone.inJail === false) {
        msg += `\n🚨 資金或信用歸零，宣告破產！`; nextState = 'END_TURN';
    }

    const activeCount = bkPlayers.filter(p => (isOfflineMode || p.uid !== null)).length;
    const aliveCount = bkPlayers.filter(p => (isOfflineMode || p.uid !== null) && !p.isBankrupt).length;
    if (activeCount > 1 && aliveCount <= 1) nextState = 'GAME_OVER';

    let nextProps = { ...gameData.properties };
    if (hasBankruptcies) {
        const bankruptIds = bkPlayers.filter(p => p.isBankrupt).map(p => p.id);
        Object.keys(nextProps).forEach(k => { if(bankruptIds.includes(nextProps[k])) delete nextProps[k]; });
    }

    await syncGameData({ players: bkPlayers, properties: nextProps, currentPlayerIdx: nextIdx, gameState: nextState, actionMessage: msg });
  };
  ...
  useEffect(() => {
    ...
    if (currAI?.isAI) {
        ...
        if (!currAI.isBankrupt || gameState === 'END_TURN') {
            ...
            else if (gameState === 'JAIL_BWA_BWEI') { tId = setTimeout(() => { const bwaLen = gameData.bwaBweiResults?.length || 0; if (bwaLen < 3) { handleThrowBwaBwei(); } else { handleFinishBwaBwei(); } }, 1200); }
            else if (gameState === 'ACTION') { tId = setTimeout(() => { const sq = BOARD_SQUARES[currAI.pos]; const hasOwner = gameData.properties?.[sq.id] !== undefined; const cB = sq.type === 'PROPERTY' && (!hasOwner) && currAI.money >= sq.price && currAI.trust >= sq.reqTrust; if (cB && Math.random() > 0.2) { handleBuyProperty(); } else { handleEndTurn(); } }, 2000); }
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#e0f2fe] overflow-hidden relative touch-none select-none font-black text-[#4a3424] flex flex-col">
      ...
      <TopBar localTimeLeft={localTimeLeft} isOfflineMode={isOfflineMode} roomId={roomId} gameData={gameData} />
      ...
      <SquareDetailsModal selectedSquareInfo={selectedSquareInfo} setSelectedSquareInfo={setSelectedSquareInfo} gameData={gameData} />

      <ActionModal isTradeActive={isTradeActive} tradeBuyer={tradeBuyer} gameData={gameData} myPlayer={myPlayer} tradeBuyerMoney={tradeBuyerMoney} handleRespondTrade={handleRespondTrade} handleThrowBwaBwei={handleThrowBwaBwei} handleFinishBwaBwei={handleFinishBwaBwei} canBuy={canBuy} handleBuyProperty={handleBuyProperty} currentSquare={currentSquare} handleEndTurn={handleEndTurn} activePlayerIndex={activePlayerIndex} />

      <AssetManagerModal showAssetManager={showAssetManager} setShowAssetManager={setShowAssetManager} myPlayer={myPlayer} myProperties={myProperties} sellProcess={sellProcess} setSellProcess={setSellProcess} handleMortgageTrust={handleMortgageTrust} handleSellToBank={handleSellToBank} initiatePlayerTrade={initiatePlayerTrade} gameData={gameData} isOfflineMode={isOfflineMode} activePlayerIndex={activePlayerIndex} />
        ...
        >
          <div className="w-full h-full p-10 bg-[#fff8e7] rounded-[4rem] shadow-[0_30px_60px_rgba(0,0,0,0.08)] border-[20px] border-[#fde047]" style={{ display: 'grid', gridTemplateColumns: 'repeat(11, 1fr)', gridTemplateRows: 'repeat(11, 1fr)', gap: '10px' }}>
            {BOARD_SQUARES.map((sq, idx) => (
               <MapSquareNode key={idx} sq={sq} idx={idx} gameData={gameData} activePlayerIndex={activePlayerIndex} myPlayer={myPlayer} inverseZoom={inverseZoom} handleRollDice={handleRollDice} setShowAssetManager={setShowAssetManager} setSelectedSquareInfo={setSelectedSquareInfo} isTradeActive={isTradeActive} dragStatus={dragStatus} />
            ))}
          </div>
        </div>
      </div>
