...
const GRID_ORDER = Array(40).fill(null).map((_, i) => {
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      } catch (e) { setErrorMsg("連線失敗，請檢查金鑰。"); }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'rooms', roomId), (docSnap) => {
      if (docSnap.exists()) {
    ...
    const moveTimer = setTimeout(async () => {
      try {
        ...
        if (gameData.remainingSteps > 0) {
          ...
          const newPos = (player.pos + 1) % 40; let newMoney = player.money; let msg = gameData.actionMessage || '';
